...
**Autoresearch loop on this dataset (2026-09-09, `autoresearch/protein-ccrcc`, report `/srv/GT/analysis/pgueguen/rctd-py/ccrcc_protein/autoresearch_report.html`)**: 20 keep/revert iterations against sqrt(held-out lineage F1 x 10x fine-type agreement) took the tune metric 0.457 -> 0.597 and the untouched TEST split 0.405 -> 0.507; protein contributes +0.068 of the final test value and clears three shuffled-marker nulls by +0.056. What moved it, in order: dropping reference types that absorb cells on a 477-gene panel (cDC1/cDC2 took the macrophages, Cycling_myeloid_cell was a top-4 call in 14 of 16 10x groups, Cycling_T_NK_cell), scaling CONFIDENCE/DOUBLET thresholds to the panel size (x0.05 of the 5k-gene defaults; rejects 28% -> 8%), `protein_singlet_purity=0.9` (exhausted CD8 recall 0.46 -> 0.70), `protein_arcsinh_cofactor=20`, `counts_MIN=3`, lambda 1.5. What hurt: curated signed gates on a SUBSET of types in hybrid mode (twice - the bootstrap types absorb the gated ones whichever side is gated), `protein_var_model="unit"`, lambda 2 before the purity change, removing the DE-gene filter, tumour k-means sub-states, and swapping T-cell types for Krishna 2021 labels (its CD8 label is naive CD8). Still open: DISCO's Treg profile absorbs 93% of 10x's helper T cells and no CD4 reference fixed it; NK needs a profile the panel can reward.
...
- **Never route continuous protein through count-fusion.** `run_rctd_multimodal([RNA, protein], fusion="count")` treats every modality as Poisson counts; fed continuous intensity (~1e6-1e7) the protein likelihood is flat and the joint collapses to a single constant weight vector across all pixels (corr ~0, dominant-type accuracy ~chance) -- confirmed on a ~62-plex CosMx dataset and a synthetic RNA-degenerate-pair test. As of `feat/protein-celltune` the default is `fusion="protein_wls"` and `"count"` REFUSES continuous input, so this trap is now closed by the code; on `feat/multimodal-protein` and earlier you must pass `fusion="protein_wls"` yourself.
- **Raise the UMI ceiling for protein-intensity values.** The RNA-count ceiling (`--umi-max` default 2e7) silently filters out protein-scale per-cell sums (e.g. CosMx protein ~3e7, ~6e7 for a 2-cell pseudospot) -- nearly all pixels get dropped and scoring crashes on a shape mismatch. When feeding protein-scale values, set the ceiling to 1e9-1e12; RNA totals are hundreds-to-thousands, so a high ceiling leaves RNA membership unchanged.
- **Measure lambda, do not guess it** (`feat/protein-celltune`). `protein_weight="landmark"` gates high-confidence cells from the protein channels, splits the markers into folds so truth and fit never share a marker, sweeps `protein_lambda_grid`, and keeps the best macro F1 -- then requires that gain to beat the largest gain from `protein_lambda_nulls` (6) column-shuffled copies of the same panel. That null is load-bearing, not ceremony: a NOISE panel scored gain +0.039 while a single shuffled sweep drew +0.038, so both "beats lambda=0" and "beats one null" picked lambda=4 on pure noise. Read `RCTD.protein_lambda_curve` -- and read the ABSOLUTE F1, not just the argmax: marker folds cannot detect a panel that is internally consistent but attached to the wrong cells (mis-registration, segmentation offset), because truth and fit then share the same wrong assignment. A selected lambda sitting on a low absolute F1 means the landmarks themselves are not trustworthy.
- **A fixed `protein_signature_magnitude` may be what a large lambda is paying for.** The default writes +/-1.5 z for every curated marker of every type. On a planted separation of z=6 the measured level is 6.6-7.1, and `protein_signature_magnitude="calibrated"` writes per-marker levels from landmark cells instead. Try that before cranking lambda: lambda scales the WHOLE panel, the magnitude scales the one marker that is mis-stated.
- **Protein spillover has a knob now.** `protein_reliability="neighbour_ratio"` downweights a cell whose protein does not stand above the same markers in its spatial neighbours (needs `obsm["spatial"]`). Before this there was no spatial term at all and the only background handling was a global per-marker constant unreachable from `RCTDConfig`. It is a per-cell weight on the protein term, so `spot_class` stays consistent with the weights.
- **Doublet mode reports where the two modalities disagree.** `DoubletResult.modality_conflict` / `.modality_confusion` (and `obs["rctd_modality_conflict"]` from the CLI) come free from decomposing the singlet score -- no extra fit. Off-diagonal mass in the confusion matrix is where to look for a cell type the reference does not contain, and it generalises the dataset-level MECR proxy to a per-cell flag.
- **`protein_weight="auto"` underweights a rich, informative panel.** The per-feature heuristic balances gradient magnitude at init, not marker informativeness -- on a deep, trustworthy panel (60+ markers) it lands low (~0.1) and only ties RNA-only, while a fixed weight ~4 can be best-in-class. Sweep it up to 4+ even for sharpening (not just to override a wrong RNA call) when the panel is deep. `protein_norm="arcsinh_robust"` handles IF intensity and 10x Xenium Protein mean-stain values alike; prefer it over per-cell CLR for IF (CLR couples the few channels via the per-cell geometric mean).
- **`class_df` (doublet mode, also on this branch) relaxes the singlet/doublet/reject confidence threshold for within-class ambiguity -- it does NOT change which type wins.** Grouping same-lineage subtypes (e.g. several fibroblast subtypes into one `Fibroblast` class) recovers more usable singlets with no accuracy downside, but it will NOT fix a wrong-type-assignment problem (that lives upstream in candidate scoring). Keep ungrouped any ambiguity you actually need to resolve.
