...
def _run_batched_scoring(
    ...
    for start in range(0, M, batch_size):
        ...
        batched_nll = jax.vmap(
            lambda y, lam: calc_log_likelihood(y, lam, Q_gpu, SQ_gpu, X_gpu, K_val),
            in_axes=(0, 0)
        )
...
def run_multi_mode(
    ...
    for k in range(1, config.MAX_MULTI_TYPES + 1):
        ...
        scores = _run_batched_scoring(
            tasks, spatial_numi, spatial_counts, P_gpu, Q_gpu, SQ_gpu, X_gpu, batch_size, config.K_val
        )
    ...
    # Confidence check
    # conf_list[type] = FALSE if swapping it with another candidate yields score < current_score + CONFIDENCE_THRESHOLD
    ...
    for k_sub, t_list in conf_tasks.items():
        ...
        scores = _run_batched_scoring(
            base_tasks, spatial_numi, spatial_counts, P_gpu, Q_gpu, SQ_gpu, X_gpu, batch_size, config.K_val
        )
